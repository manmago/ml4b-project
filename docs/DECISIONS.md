...
**Decision.** Ship **two** models and let the app run both on every upload:
...
- **Model 2 (current)** — trained on Kaggle **+ our committed Testdaten**
  (`best_model.joblib` + `novelty_detector.joblib` + `model_metrics.json`).

The Predict page runs both on the same recording and shows the timelines,
distributions and a per-window comparison side by side; the Model Performance page
shows their CV macro/per-class F1 next to each other. This makes the value of our
own continual-learning data **visible and honest** instead of asserted.

**Augmentation is applied identically to both.** Our own Testdaten is concatenated
with the Kaggle anchor *before* the 6× augmentation (§4), so the uploaded data gets
the same rotation/time-warp/mirror/jitter as the anchor. Both models are produced by
one shared module (`src/ml4b/models/pipeline.py`) — identical windowing, augmentation,
features, Random Forest hyper-parameters and novelty calibration — so the **only**
difference between them is the data they see. That is what makes the comparison fair
(the project's single-pipeline rule extended to the two-model build).

**Why a frozen baseline, regenerated by the same build.** `make update` rebuilds both
models in one deterministic run, so the baseline never drifts and stays comparable to
the current model on the same commit. `make train` (Kaggle-only bootstrap) writes the
Kaggle model into *both* slots, so before any Testdaten is added the two models are
identical — which honestly reads as "no own-data effect yet".

**Honest caveat surfaced in the app.** The two CV macro-F1 numbers are **not** a clean
apples-to-apples improvement: each is evaluated over its own held-out sets, and Model
2's held-out pool also contains our own (cross-person, harder) recordings — so the
aggregate score can even dip slightly. The clean, like-for-like comparison is the
**same-recording** view on the Predict page. Both pages state this.
