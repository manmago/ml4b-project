...
Open the **Terminal** app (or iTerm) and run:
...
#    source ~/.zshrc), otherwise you get "uv: command not found".  Check: uv --version
...
If `uv` is not found after install, restart Terminal or `source $HOME/.local/bin/env`.
...
1. Open the project: `code .` (if `code` is missing: `Cmd+Shift+P` →
   **Shell Command: Install 'code' command in PATH**).
2. Install the **Python**, **Jupyter** and **Ruff** extensions.
3. Select the interpreter `./.venv/bin/python` (created on the first `uv run`).
4. Run dev tools with `make test`, `make lint`, `make format`.
</details>
...
pbcopy < ~/.ssh/id_ed25519.pub                 # add this key at GitHub → Settings → SSH keys
...
| Symptom | Fix |
|---------|-----|
| `uv: command not found` | Restart Terminal or `source $HOME/.local/bin/env`. |
...
| Apple Silicon build issues | uv installs native arm64 wheels automatically — just re-run. |
