...
└── feedback/              ← User label corrections for continual learning
                              (feedback.jsonl; ADR-027). NOT in git — the user's own data.
```
...
└── saved/
    ...
    │                             model so it can be restored (ADR-027). NOT in git.
    └── model_manifest.json     ← generated by retrain — what feedback went into the
                                current model (ADR-027). NOT in git.
```
...
├── feedback/                 ← Human-in-the-loop continual learning (ADR-027)
│   ├── __init__.py
...
├── update_model.py          ← Continual learning: retrain on base data + user corrections (ADR-027).
...
├── test_feedback.py           ← feedback store round-trip, stats, pipeline compatibility (ADR-027)
